...
router.get('/randomUser', (req, res) => {
  try {
    ...
    let company = faker.company.name().replace(/\s+/g, '').replace('.', '').toLowerCase();
...
let query = 'SELECT u.id, u.username, u.email, p.birthdate, p.gender, p.maritalstatus, p.jobtitle, p.phone FROM users u JOIN persons p ON u.id = p.users_Id;';      
  try {
...
router.put('/users/:id', async (req, res) => {
  ...
  try {
    const [result] = await pool.execute('UPDATE cats SET name = ?, owner = ?, birth = ? WHERE id = ?', [
      name.trim(),
      owner.trim(),
      birth.trim(),
      id,
